    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden pb-safe">
            <div className="bg-slate-950/90 backdrop-blur-xl border-t border-white/10 px-6 py-3 flex justify-between items-center relative">
                ...
                <div 
                    className="absolute top-0 h-0.5 bg-emerald-500 transition-all duration-300 ease-out"
                    style={{ 
                        width: '20%', 
                ...
                {tabs.map((tab) => {
                    ...
                    return (
                        ...
                        >
                            <div className={`p-1 rounded-full transition-all duration-300 ${isActive ? 'bg-emerald-500/10' : ''}`}>
                                <Icon className={`w-6 h-6 ${isActive ? 'fill-emerald-500/20' : ''}`} />
                            </div>
